...
undoModule = (function() {
  ...
  function reportEvent(event) {
    ...
    if ((! event.backset) || Changeset.isIdentity(event.backset)) {
      ...
      // Remove the branch ahead of the current pointer.
      var curPointer = stack.numEvents() - undoPtr;
      while (stack.numEvents() > curPointer) {
        stack.popEvent();
      }
      if (! merged) {
  ...
  function canPerformUndo() {
    return undoPtr < stack.numEvents() - 1;
  }
  ...
  return {clearHistory:clearHistory, reportEvent:reportEvent, reportExternalChange:reportExternalChange,
    ...
    canPerformUndo:canPerformUndo, canPerformRedo:canPerformRedo,
