...
(function ($) {
  ...
  var REGEX_TMP = /[-:@a-zA-Z0-9_.,~%+\/\\?=&#;()'$]/;
  var REGEX_URLCHAR = new RegExp('('+REGEX_TMP.source+'|'+REGEX_WORDCHAR.source+')');
  var REGEX_URL = new RegExp(/(?:(?:https?|sftp|ftps?|ssh|ircs?|file|gopher|telnet|nntp|worldwind|chrome|chrome-extension|svn|git|mms|smb|afp|nfs|(x-)?man|gopher|txmt):\/\/|mailto:|xmpp:|sips?:|tel:|sms:|news:|bitcoin:|magnet:|urn:|geo:)/.source+REGEX_URLCHAR.source+'*(?![:.,;\\)\'])'+REGEX_URLCHAR.source, 'g');
  ...
  function Handsontable(container, settings) {
    ...
    grid = {
      ...
      alter: function (action, coords, toCoords) {
        ...
        // HACKPAD: alter auto-row management behavoir
        grid.suspendEmptyRowManagement();
        // HACKPAD: alter auto-row management behavoir
      ...
      // HACKPAD: alter auto-row management behavoir
      suspendEmptyRowManagement: function () {
        self.emptyRowManagementSuspended = true;
      },
      resumeEmptyRowManagement: function () {
        self.emptyRowManagementSuspended = false;
      },
      // HACKPAD: alter auto-row management behavoir
      ...
      keepEmptyRows: function () {
        ...
        // HACKPAD: alter auto-row management behavoir
        if (self.emptyRowManagementSuspended) {
          return;
        }
        // HACKPAD: alter auto-row management behavoir
...
//        }
    ...
    selection = {
      ...
      refreshBorders: function () {
        ...
        // hackpad change: never destroy and active editing box
      ...
      // hackpad change: bold/italic/underline
      /**
       * Runs callback for each cell coord in selection and updates cells according to returned value
       */
      eachCell: function (callback, label) {
        var changes = [];
        var startRow = Math.min(selection.end().row, selection.start().row);
        var endRow = Math.max(selection.end().row, selection.start().row);
        var startCol = Math.min(selection.end().col, selection.start().col);
        var endCol = Math.max(selection.end().col, selection.start().col);

        for (var i=startRow; i<endRow+1; i++) {
          for (var j=startCol; j<endCol+1; j++) {
            var oldVal = self.getData(true)[i][j];
            var newVal = callback(i, j, oldVal);
            changes.push([i,j, oldVal, newVal]);
            self.setDataAtCell(i, j, newVal, true/*allowHTML*/);
          }
        }

        self.container.triggerHandler("datachange.handsontable", [changes, label]);
      },
      // hackpad change: bold/italic/underline
    ...
    // hackpad change: bold/italic/underline
    function getAttribs(val) {
      if (!val) {
        return [];
      }
      var allAttribs = ['b', 'i', 'u', 'del'];
      var attribs = [];
      for (var i=0; i<allAttribs.length; i++) {
        if (val.match(new RegExp("<"+allAttribs[i]+">.*<\/"+allAttribs[i]+">"))) {
          attribs.push(allAttribs[i]);
        }
      }
      return attribs;
    }
    function stripAttribs(val) {
      var attribs = getAttribs(val);
      for (var i=0; i<attribs.length; i++) {
        val = toggleAttrib(val, attribs[i]);
      }
      return val;
    }
    function setAttribs(val, attribs) {
      for (var i=0; i<attribs.length; i++) {
        val = toggleAttrib(val, attribs[i]);
      }
      return val;
    ...
    function toggleAttrib(val, attr) {
      if (attr == "") {
        return val;
      }
      if (val.match(new RegExp("<"+attr+">.*<\/"+attr+">"))) {
        return val.replace(new RegExp("(.*)<"+attr+">(.*)<\/"+attr+">(.*)"), function (match, grp1, grp2, grp3) {
    ...
    editproxy = {
      ...
      init: function () {
            ...
            if ((event.keyCode == 32) || //space
              ...
              if (!priv.isCellEdited && !ctrlDown) { //disregard CTRL-key shortcuts
                ...
                // hackpad change: bold/italic/underline
                  ...
                  // Bold
                  selection.eachCell(function (i, j, val) {
                    var x =  toggleAttrib(val, "b");
                    return x;
                  }, "bold");
                }
                  ...
                  // Italic
                  selection.eachCell(function (i, j, val) {
                    return toggleAttrib(val, "i");
                  }, "italic");
                }
                else if (event.keyCode === 85) { //CTRL + U
                  // Underline
                  selection.eachCell(function (i, j, val) {
                    return toggleAttrib(val, "u");
                  }, "underline");
                }
                // end hackpad change
        ...
        // hackpad change: bold/italic/underline
        priv.editProxy.triggerAttrToggle = function(attr) {
          selection.eachCell(function (i, j, val) {
            return toggleAttrib(val, attr);
          });
        }
        // end hackpad change
      ...
      beginEditing: function (useOriginalValue, suffix) {
        ...
        if (useOriginalValue) {
          ...
          // hackpad change: bold/italic/underline
          // we strip the attibs when going into editing textbox
          var attrs = getAttribs(original);
          for (var i=0; i<attrs.length; i++) {
            original = toggleAttrib(original, attrs[i])
          }
          // end hackpad change
      ...
      finishEditingCells: function (row, col, val) {
        ...
        // hackpad change: bold/italic/underling
        // re-apply attributes after edit
        val = setAttribs(val, getAttribs(oldVal));
        // end change
    ...
    interaction = {
      ...
      // hackpad change: crappy but workable ios editing
      onCellTouchEnd: function () {
        if (selection.touchMoved) {
          return false;
        }
        editproxy.finishEditing();
      ...
      onCellTouchStart: function () {
        selection.touchMoved = false;
      },
      onCellTouchMove: function () {
        selection.touchMoved = true;
      },
      // end hackpad change
    ...
    this.init = function () {
      ...
      if (!('ontouchstart' in document.documentElement)) {
        ...
        self.table.on('touchstart', 'td', interaction.onCellTouchStart);
        self.table.on('touchmove', 'td', interaction.onCellTouchMove);
        self.table.on('touchend', 'td', interaction.onCellTouchEnd);
      }
      // end hackpad change
      ...
      if (priv.settings.contextMenu) {
        var onContextClick = function (key) {
          ...
          // hackpad change: prevent user from deleting the last row/col
          if (priv.selStart.row >= (datamap.getAll().length-1) ||
            priv.selEnd.col >= (datamap.getAll()[0].length-1)) {
            return;
          }
          // end hackpad change
          ...
          switch (key) {
            ...
            case "remove_table":
              if (priv.settings.onDeleteTable) {
                priv.settings.onDeleteTable();
              }
              break;
        ...
        var allItems = {
          ...
          "sep3": "---------",
          "remove_row": {name: "Delete row", disabled: isDisabled},
          "remove_col": {name: "Delete column", disabled: isDisabled},
          "remove_table": {name: "Delete table", disabled: function() {
            return !priv.settings.onDeleteTable;
          }}
        };

        if (priv.settings.contextMenu === true) { //contextMenu is true, not an array
          priv.settings.contextMenu = ["row_above", "row_below", "sep2", "col_left", "col_right", "sep3", "remove_row", "remove_col", "remove_table"]; //use default fields array
        }
        ...
        $.contextMenu({
          ...
          // hackpad change: work-around frame nesting
          appendTo: priv.settings.contextMenuContainer,
          offsetFunc: priv.settings.contextMenuOffsetFunc ,
          // end hackpad change
          trigger: 'right',
      ...
      self.container.on("datachange.handsontable", function (event, changes, source) {
        if (priv.settings.onChange) {
          // hackpad change: send attribs separately from values
          for (var i=0; i<changes.length;i++) {
            var attribs = getAttribs(changes[i][3]);
            var oldAttribs = getAttribs(changes[i][2]);
            changes[i][3] = stripAttribs(changes[i][3]);
            changes[i][4] = attribs; //new attribs
            changes[i][5] = oldAttribs; //old attribs
          }
          // hackpad change
    ...
    this.keepEmptyRows = function () {
      grid.keepEmptyRows();
    };

    // hackpad change: bold/italic/underline
    this.triggerAttrToggle = function (attr) {
      priv.editProxy.triggerAttrToggle(attr);
    };
    // end change


    /**
     * Set attribute at given cell
     * @public
     * @param {Number} row
     * @param {Number} col
     * @param {String} attrib string ('b','s','u','del',..)
     * @param {Boolean} [allowHtml]
     */
    this.setAttribAtCell = function (row, col, attrib) {
      self.setDataAtCell(row, col, toggleAttrib(self.getData(true)[row][col], attrib));
    }
    ...
    this.setDataAtCell = function (row, col, values, allowHtml) {
      ...
      for (var i = 0, ilen = values.length; i < ilen; i++) {
        ...
        if (!allowHtml) {
          // hackpad change
          var attribs = getAttribs(value);
          escaped = stripAttribs(value).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;"); //escape html special chars
          escaped = setAttribs(escaped, attribs);
          // end hackpad change (attribs)
        }

        // hackpad change: linkify URLs
        if (!allowHtml && escaped.match(REGEX_URL)) {
          td.style.wordBreak = "break-all";
          escaped = '<a target="_blank" href="'+escaped+'">' + escaped.match(REGEX_URL)[0] + '</a>';
        }
        // end hackpad change
...
handsontable.UndoRedo.prototype.isUndoAvailable = function () {
  // hackpad change: allow undo of first rev
  return (this.rev >= 0);
  // end change (this.rev > 0)
};
